...
API void symbol_lower(const uint8_t*codes,int64_t n,int d,int nsfa,const double*lookup,double*out){
    for(int64_t i=0;i<n;i++){
        double a=0,b=0;
        for(int j=0;j<nsfa;j++)a+=lookup[j*256+codes[i*d+j]];
        for(int j=nsfa;j<d;j++)b+=lookup[j*256+codes[i*d+j]];
        out[i]=a>b?a:b;
    }
}
