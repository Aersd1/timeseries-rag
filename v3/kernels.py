...
LIB.symbol_lower.argtypes=[U,L,C,C,F,F]
LIB.symbol_lower.restype=None
...
def symbol_lower(codes,lookup,nsfa):
    codes=np.ascontiguousarray(codes);out=np.empty(len(codes))
    LIB.symbol_lower(codes,len(codes),codes.shape[1],nsfa,np.ascontiguousarray(lookup),out)
    return out
