...
`v6-bayesian-future-retrieval` 在用户上传的 V5 代码与实验结果上新增独立的 `v6/`：条件未来概率模型 → 概率引导编码器 → 多层向量检索 → 历史后续片段预测。

- [V6 结构、逻辑与复杂度](v6/README.md)
- [服务器训练、迁移、评估与结果回传](v6/SERVER_GUIDE.md)
- [论文依据与 V5 结果复查](v6/RESEARCH.md)
- [代码审查和测试范围](v6/REVIEW.md)
- [服务器结果复查、本轮优化与实测](v6/OPTIMIZATION.md)
- **[当前主方案：历史与未来同时相似的联合检索](v6/JOINT_RETRIEVAL.md)**

本地完成了正确性测试与服务器已有权重的推理优化对照，新编码训练效果仍待服务器验证。本分支继承 V5/V6 的实验文件，建议按 V6 指南稀疏克隆。以下 V2/V3 说明保留作旧版本参考，其中不包含实验数据的描述仅适用于旧版本。

## 当前推荐版本：V3

新增 [V3 非重叠检索](v3/README.md)：同一文件、列、设备内返回片段的起点至少相隔 244 点；默认预取 512 个候选、上限 8192 个，按块核验，不保存全库查询距离。候选上限不足时明确返回未完成状态。

- [冗余预取、空间限制与测试报告](v3/REDUNDANCY_REPORT.md)
- [最新测试指标](v3/docs/benchmarks/redundancy/metrics.json)
- [耗时与缓存对比图](v3/docs/benchmarks/redundancy/comparison.png)

根目录保留 V2，V3 源码位于 `v3/`，需在该目录单独编译本地内核。已有 V2 完整索引可复用；新数据建库见 [迁移指南](MIGRATION_GUIDE.md)。以下为 V2 的原使用说明。
